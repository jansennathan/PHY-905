...
//  I wouls assume this is because we are using a HO basis which is bounded so if you try to extend the length it
...
main ()
{
  ...
  for (int step = 1; step < 2; step++){
    ...
    int dimension = 35;		// dimension of the matrices and vectors 
    ...
    for (int i = 0; i < dimension; i++)
      {
        ...
        if (i == 0) {
         ...
         wavefunction << n << "      " << abs(psi) << "     "  << wavereal << "     "  << error << endl;
         }
        }
        wavefunction.close();
      }
